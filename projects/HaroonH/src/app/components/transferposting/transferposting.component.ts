...
export class TransferpostingComponent implements OnInit {
  ...
  getJobs() {
    ...
    this.http
      .get(this.serverUrl + "api/getJobs", { headers: reqHeader })
      .subscribe((data: any) => {
        this.jobsList = data;
  ...
  add() {
    ...
    if (this.postTo == undefined || this.postTo == "") {
      ...
      this.JobDesigIDFrom == this.JobDesigIDTo &&
      this.JobPostDeptCdFrom == this.JobPostDeptCdTo &&
      ...
      if (dataList.length > 0) {
        this.toastr.errorToastr(
          dataList[0].EmpName + " Already Added",
          "Error",
          { toastTimeout: 2500 }
        );
        return false;
        ...
        .subscribe((data: any) => {
          ...
          for (var i = 0; i < this.transfersList.length; i++) {
            for (var j = 0; j < this.transfersList.length; j++) {
              if (
                this.transfersList[i].JobDesigID2 ==
                  this.transfersList[j].JobDesigID &&
                this.transfersList[i].JobPostDeptCd2 ==
                  this.transfersList[j].JobPostDeptCd &&
                this.transfersList[i].JobPostLocationCd2 ==
                  this.transfersList[j].JobPostLocationCd
              ) {
    ...
    if (filterOption == "filtersection1") {
      dataList = this.jobsList.filter(
        x =>
          ...
          x.managerJobPostDeptCd == this.department1
      );
      this.tempJobList1 = dataList;
    ...
    if (filterOption == "filtersection2") {
      dataList = this.jobsList.filter(
        x =>
          ...
          x.managerJobPostDeptCd == this.department2
      );
      this.tempJobList2 = dataList;
