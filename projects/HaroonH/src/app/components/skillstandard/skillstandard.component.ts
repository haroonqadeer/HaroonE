...
export class SkillstandardComponent implements OnInit {
  ...
  getSkillStandard() {
    ...
    this.http
      .get(this.serverUrl + "api/getSkillStandard", { headers: reqHeader })
      .subscribe((data: any) => {
  ...
  getSkillStandardDetails() {
    ...
    this.http
      .get(this.serverUrl + "api/getSkillStandardDetails", {
        headers: reqHeader
      })
      .subscribe((data: any) => {
  ...
  saveSkillStandards() {
    if (this.skillGroup == "") {
      ...
      this.toastr.errorToastr(
        "Please Enter Required Marks from 0-5 ",
        "Error",
        { toastTimeout: 2500 }
      );
      return false;
    } else if (
      parseInt(this.qlfctnCriteriaReqdLvl) > 5 ||
      parseInt(this.qlfctnCriteriaReqdLvl) < 0
    ) {
      this.toastr.errorToastr("Please Enter Number between 0-5 ", "Error", {
        toastTimeout: 2500
      });
      return false;
    } else {
      ...
      if (this.skillStandardId != "") {
        ...
        this.http
          .put(this.serverUrl + "api/updateSkillStandard", updateData, {
            headers: reqHeader
          })
          .subscribe((data: any) => {
            //alert(data.msg);
        ...
        //alert(reqHeader);
        this.http
          .post(this.serverUrl + "api/saveSkillStandard", saveData, {
            responseType: "json"
          })
          .subscribe((data: any) => {
  ...
  newDetailList(item) {
    ...
    for (var i = 0; i < this.skillStandardDetailsList.length; i++) {
      ...
      if (
        this.skillStandardDetailsList[i].qlfctnName == item.qlfctnName &&
        this.skillStandardDetailsList[i].qlfctnCriteriaName ==
          item.qlfctnCriteriaName
      ) {
        this.newSkillDataList.push({
          qlfctnName: this.skillStandardDetailsList[i].qlfctnName,
          qlfctnCriteriaName: this.skillStandardDetailsList[i]
            .qlfctnCriteriaName,
          jobDesigName: this.skillStandardDetailsList[i].jobDesigName,
          qlfctnCriteriaReqdLvl: this.skillStandardDetailsList[i]
            .qlfctnCriteriaReqdLvl,
          qlfctnCriteriaMaxLvl: this.skillStandardDetailsList[i]
            .qlfctnCriteriaMaxLvl,
          reqdQlfctnRuleNo: this.skillStandardDetailsList[i].reqdQlfctnRuleNo,
          qlfctnRuleCriteriaCd: this.skillStandardDetailsList[i]
            .qlfctnRuleCriteriaCd
        });
      }
  ...
  delete() {
    ...
    if (this.userPassword == "") {
      ...
      if (this.dSkillStandardId != "" && this.dSkillReqdStandardId != "") {
        ...
        this.http
          .put(this.serverUrl + "api/deleteSkillStandard", data, {
            headers: reqHeader
          })
          .subscribe((data: any) => {
            //alert(data.msg);
  ...
  //       completeDataList,
  //       new IgxCsvExporterOptions("skillStandardCompleteCSV", CsvFileTypes.CSV)
  //     );
  //   }
  ...
  //     for (var i = 0; i < this.skillStandardList.length; i++) {
  //       if (
  //         this.skillStandardList[i].qlfctnName
  //           .toUpperCase()
  //           .includes(this.tblSearch.toUpperCase()) ||
  //         this.skillStandardList[i].qlfctnCriteriaName
  //           .toUpperCase()
  //           .includes(this.tblSearch.toUpperCase())
  //       ) {
  ...
  //       this.csvExportService.exportData(
  //         filteredDataList,
  //         new IgxCsvExporterOptions("skillStandardFilterCSV", CsvFileTypes.CSV)
  //       );
  //     } else {
  ...
  //       new IgxExcelExporterOptions("skillStandardCompleteExcel")
  //     );
  //     this.excelDataList = [];
  //   }
  //   // case 2: When tblSearch is not empty then assign new data list
  ...
  //       this.excelExportService.export(
  //         this.excelDataContent,
  //         new IgxExcelExporterOptions("skillStandardFilterExcel")
  //       );
  //       this.excelDataList = [];
  //     } else {
  //       this.toastr.errorToastr("Oops! No data found", "Error", {
