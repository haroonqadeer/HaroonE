...
//   serverUrl = "https://localhost:8008/";
  ...
  addGoals() {
    if (this.startDate == undefined || this.startDate == "") {
      ...
      this.toastr.errorToastr('Invalid start Date', 'Error', { toastTimeout: (2500) });
      return false;
    }
    ...
    // }
      ...
      for (var i = 0; i < this.tempProjectsPerformanceData.length; i++) {
        if (this.tempProjectsPerformanceData[i].SupervisorId == this.supervisorName) {
  ...
  addSkills() {
    ...
    if (this.skillDate == undefined || this.skillDate == "") {
      ...
      for (var i = 0; i < this.skillsList.length; i++) {
        if (this.skillsList[i].SkillCriteriaCd == this.skillCriteriaCd && this.skillsList[i].SkillDate == this.skillDate) {
          duplicateChk = true;
        }
      }
  ...
  delete() {
    ...
    if (this.userPassword == "") {
      ...
      if (this.dProjectsId != "") {
        ...
        this.http.put(this.serverUrl + 'api/deleteProjects', deleteData, { headers: reqHeader }).subscribe((data: any) => {
  ...
  downloadCSVGoals() {
    ...
    if (this.tblSearchGoals == "") {
      ...
      for (var i = 0; i < this.tempProjectsPerformanceData.length; i++) {
        ...
        completeDataList.push({
          ...
          RateSprvsrAssessment: this.tempProjectsPerformanceData[i].RateSprvsrAssessment
        });
      }
    ...
    // case 2: When tblSearchType is not empty then assign new data list
      ...
      var filteredDataList = [];
      for (var i = 0; i < this.tempProjectsPerformanceData.length; i++) {
        if (this.tempProjectsPerformanceData[i].SupervisorName.toUpperCase().includes(this.tblSearchGoals.toUpperCase())) {
          filteredDataList.push({
            StartDate: this.tempProjectsPerformanceData[i].StartDate,
            FinishDate: this.tempProjectsPerformanceData[i].FinishDate,
            ProjectDesc: this.tempProjectsPerformanceData[i].ProjectDesc,
      ...
      if (filteredDataList.length > 0) {
        ...
        this.toastr.errorToastr('Oops! No CSV data found', 'Error', { toastTimeout: (2500) });
      }
    }
  }
  ...
  downloadExcelGoals() {
    ...
    // case 2: When tblSearchType is not empty then assign new data list
      ...
      if (this.excelDataListGoals.length > 0) {
        ...
        this.toastr.errorToastr('Oops! No Excel data found', 'Error', { toastTimeout: (2500) });
      }
    }
  }
  ...
  downloadCSVSkills() {
    ...
    // case 2: When tblSearchType is not empty then assign new data list
      ...
      var filteredDataList = [];
      for (var i = 0; i < this.skillsList.length; i++) {
        if (this.skillsList[i].QlfctnSkillName.toUpperCase().includes(this.tblSearchSkills.toUpperCase())) {
          filteredDataList.push({
            SkillDate: this.skillsList[i].SkillDate,
            SkillCriteriaCd: this.skillsList[i].SkillCriteriaCd,
            RateSkillLevel: this.skillsList[i].RateSkillLevel,
