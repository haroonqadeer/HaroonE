...
export class TrainingrequirementsComponent implements OnInit {
  ...
  getTrainingRequirements() {
    ...
    this.http
      .get(this.serverUrl + "api/getTrainingRequirements", {
        headers: reqHeader
      })
      .subscribe((data: any) => {
  ...
  getTrainingInstitute() {
    ...
    this.http
      .get(this.serverUrl + "api/getTrainingInstitute", { headers: reqHeader })
      .subscribe((data: any) => {
  ...
  assignTraining() {
    ...
    if (this.tTrainingType == "") {
      ...
      this.toastr.errorToastr("Please Select Training Name", "Error", {
        toastTimeout: 2500
      });
      return false;
      ...
      this.toastr.errorToastr("Please Select Start Training Date", "Error", {
        toastTimeout: 2500
      });
      return false;
    } else if (this.endDate == "") {
      this.toastr.errorToastr("Please Select End Training Date", "Error", {
        toastTimeout: 2500
      });
      return false;
    } else if (this.selectedList.length < 0 || this.selectedList.length == 0) {
      this.toastr.errorToastr("Oops! No Employee for Training", "Error", {
        toastTimeout: 2500
      });
      return false;
    } else {
      if (this.tTrainingId != "") {
        ...
        //alert(reqHeader);
        this.http
          .post(this.serverUrl + "api/saveEmpJobTraining", saveData, {
            responseType: "json"
          })
          .subscribe((data: any) => {
  ...
  downloadPDF() {
    ...
    //     doc.text(
    //       "Office # 8,9, 3rd Floor Anayat Plaza, G11 Markaz Islamabad.",
    //       80,
    //       25
    //     );
  ...
  //         SkillDifference:
  //           this.trainingRequirementList[i].qlfctnCriteriaReqdLvl -
  //           this.trainingRequirementList[i].levelAchieved
  //       });
  //     }
  //     this.csvExportService.exportData(
  //       completeDataList,
  //       new IgxCsvExporterOptions(
  //         "trainingRequirementCompleteCSV",
  //         CsvFileTypes.CSV
  //       )
  //     );
  //   }
  ...
  //     for (var i = 0; i < this.trainingRequirementList.length; i++) {
  //       if (
  //         this.trainingRequirementList[i].indvdlFullName
  //           .toUpperCase()
  //           .includes(this.tblSearch.toUpperCase()) ||
  //         this.trainingRequirementList[i].jobDesigName
  //           .toUpperCase()
  //           .includes(this.tblSearch.toUpperCase())
  //       ) {
  ...
  //       new IgxExcelExporterOptions("trainingRequirementCompleteExcel")
  //     );
  //     this.excelDataList = [];
  //   }
  //   // case 2: When tblSearch is not empty then assign new data list
  ...
  //       this.excelExportService.export(
  //         this.excelDataContent,
  //         new IgxExcelExporterOptions("trainingRequirementFilterExcel")
  //       );
  //       this.excelDataList = [];
  //     } else {
  //       this.toastr.errorToastr("Oops! No data found", "Error", {
