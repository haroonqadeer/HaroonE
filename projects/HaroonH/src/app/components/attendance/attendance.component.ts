...
export class AttendanceComponent implements OnInit {
  ...
  getBreakType() {
    ...
    this.http
      .get(this.serverUrl + "api/getBreakType", { headers: reqHeader })
      .subscribe((data: any) => {
  ...
  public getDepartment() {
    if (this.app.locationId == undefined) {
      setTimeout(() => {
        ...
        this.http
          .post(this.serverUrl + "api/getDepartment", reqData, {
            headers: reqHeader
          })
          .subscribe((data: any) => {
  ...
  getSection() {
    if (this.app.locationId == undefined) {
      ...
      this.http
        .post(this.serverUrl + "api/getSection", reqData, {
          headers: reqHeader
        })
        .subscribe((data: any) => {
          for (var i = 0; i < data.rows.length; i++) {
  ...
  getDeptEmp() {
    ...
    this.http
      .post(this.serverUrl + "api/getDeptEmp", reqData, { headers: reqHeader })
      .subscribe((data: any) => {
  ...
  getSectEmp() {
    ...
    this.http
      .post(this.serverUrl + "api/getSectEmp", reqData, { headers: reqHeader })
      .subscribe((data: any) => {
        this.employeeList = data.rows;
  ...
  save() {
    if (this.ddlDepartment == undefined || this.ddlDepartment == "") {
      ...
      this.toastr.errorToastr("Please enter time", "Error", {
        toastTimeout: 2500
      });
      return false;
    } else if (
      this.chkAddBreak == true &&
      (this.ddlBreakType == undefined || this.ddlBreakType == "")
    ) {
      this.toastr.errorToastr("Please enter break type", "Error", {
        toastTimeout: 2500
      });
      return false;
    } else {
  ...
  getFilterItem(filterOption) {
    ...
    if (filterOption == "section") {
      ...
      dataList = this.tempSectnList.filter(
        x => x.parentDeptCd == this.ddlDepartment
      );
    ...
    if (filterOption == "empBreak") {
      dataList = this.empBreakList.filter(
        x =>
          x.deptCd == this.myDeptCd &&
          x.empID == this.EmpId &&
          x.dtID == this.myDtID &&
          x.shiftCd == this.myShifhCd
      );
