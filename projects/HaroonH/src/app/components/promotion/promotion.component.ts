...
export class PromotionComponent implements OnInit {
  ...
  getAvailablePosts() {
    ...
    this.http
      .get(this.serverUrl + "api/getAvailablePosts", { headers: reqHeader })
      .subscribe((data: any) => {
  ...
  save() {
    if (this.ddlJobPost == undefined || this.ddlJobPost == "") {
      this.toastr.errorToastr("Please select job post", "Error", {
        toastTimeout: 2500
      });
      return false;
      ...
      this.http
        .post(this.serverUrl + "api/promoteEmployee", saveData, {
          headers: reqHeader
        })
        .subscribe((data: any) => {
  ...
  getFilterItem(filterOption) {
    ...
    if (filterOption == "promote") {
      dataList = this.availablePostsList.filter(
        x => x.value == this.ddlJobPost
      );
